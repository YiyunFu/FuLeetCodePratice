...
struct DataObject: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        var dictString = data.replacingOccurrences(of: "=", with: "\":\"")
        dictString = dictString.replacingOccurrences(of: ", ", with: "\", \"")
        dictString = "\"\(dictString)\""
        dictString = dictString.replacingOccurrences(of: ", \"key\"", with: "}, {\"key\"")
        dictString = "[{\(dictString)}]"
        
        let dictData = dictString.data(using: .utf8)
        let arDict = try! JSONDecoder().decode([[String: String]].self, from: dictData!)
        var ages = [Age]()
        for dict in arDict {
            let encoded = try JSONEncoder().encode(dict)
            let decoded = try JSONDecoder().decode(Age.self, from: encoded)
            ages.append(decoded)
        }
        self.ages = ages
    }
    
    func countOfGreaterThan50() -> Int {
        let arAge = ages.compactMap({ Int($0.age) }).filter({ $0 >= 50 })
        return arAge.count
    }
    
    func challengeToken() -> String {
        let countOf = String(countOfGreaterThan50())
        let tokens = Array("ea14cow9k2")
        var result = ""
        for i in 0..<countOf.count {
            let strCount = Array(countOf)[i]
            let token = i<tokens.count ? String(tokens[i]) : ""
            result.append("\(strCount)\(token)")
            if i == countOf.count-1 {
                result.append(String(tokens[i+1..<tokens.count]))
            }
        }
        return result
    }
...
print(dataObject.ages.compactMap({ Int($0.age) }).count)
print(dataObject.challengeToken())
